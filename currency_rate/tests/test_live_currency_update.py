...
class CurrencyTestCase(TransactionCase):
    ...
    def test_live_currency_update(self):
        ...
        company_oe = self.env['res.company'].create({'name': 'TEST OpenExchange', 'currency_provider': 'openexchange'})
        ...
        res_oe = company_oe._update_currency_openexchange()
        ...
        self.assertEqual(rates_number + int(res_ecb) + int(res_yah) + int(res_oe), rates_number_again)
