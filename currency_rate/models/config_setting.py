...
class ResCompany(models.Model):
    ...
    def update_currency_rates(self):
        ...
        for company in self:
            ...
            elif company.currency_provider == 'openexchange':
    ...
    def _update_currency_openexchange(self):
        ''' This method is used to update the currencies by using Yahoo service provider.
            Rates are given against the company currency, which will be also updated to a rate of 1
        '''
        Currency = self.env['res.currency']
        ...
        for company in self:
            ...
            for c in currencies:
                if rates[c.name]:
                    CurrencyRate.create({'currency_id': c.id, 'rate': rates[c.name], 'name': fields.Datetime.now(), 'company_id': company.id})
        return True
